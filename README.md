...
## Senior-Level AI 실험 파이프라인

노트북에는 `run_full_ai_navigation_experiment()` 함수가 추가되어 quick mode 기준으로 전체 실험 산출물을 한 번에 생성합니다.

실행 흐름:

1. `results/` 폴더 구조 생성
2. `experiment_config.json` 저장
3. 데이터셋 메타데이터 저장
4. 기능 무결성 검사 실행
5. CNN 평가 요약 저장
6. 예측 신뢰도 분석
7. 알고리즘 비교 결과 저장
8. `risk_weight` 민감도 실험
9. confidence와 경로 성능 관계 분석
10. 실패 사례 분석
11. 강화 Ablation Study
12. AI navigation score 계산
13. 로봇 명령어 변환 검증
14. AI 실험 리포트 생성
15. 생기부용 요약 생성
...
Colab에서 노트북을 위에서부터 실행하면 결과 파일은 다음 구조로 저장됩니다.
...
│   ├── pipeline_run_status.csv
│   ├── risk_weight_sensitivity.csv
│   ├── confidence_path_relation.csv
│   ├── failure_case_analysis.csv
│   ├── ablation_study_enhanced.csv
│   ├── ai_navigation_score.csv
│   ├── robot_command_validation.csv
│   └── sensor_schema_validation.csv
├── figures/
│   ├── risk_weight_sensitivity.png
│   ├── confidence_path_relation.png
│   ├── ablation_study_enhanced.png
│   └── sensor_risk_timeseries.png
├── models/
│   └── best_cnn_model.pt
└── reports/
    ├── AI_experiment_report.md
    ├── student_record_summary.md
    ├── risk_weight_sensitivity_summary.md
    ├── confidence_path_relation_summary.md
    ├── failure_case_summary.md
    └── ablation_study_interpretation.md
```

PyTorch 기반 모델은 Keras 전용 형식이 아니라 `.pt` 형식으로 저장됩니다.

## Senior 파이프라인 실행 방법

1. Google Colab에서 `notebooks/micromouse_maze_알고리즘.ipynb`를 엽니다.
2. 위에서부터 순서대로 셀을 실행합니다.
3. 기본값인 `quick_mode=True` 상태에서 senior-level 파이프라인을 실행합니다.
4. `results/reports/AI_experiment_report.md`를 확인합니다.
5. `results/reports/student_record_summary.md`를 생기부용 활동 요약 초안으로 참고합니다.
